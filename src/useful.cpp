...
template<typename T>
void writeData_host_template(const std::string& FileName, thrust::host_vector<T>& outputData,cuint FileSize)
{
...
template<typename T>
void writeData_host_template(const std::string& FileName, std::vector<T>& outputData,cuint FileSize)
{
	std::ofstream fout(FileName.c_str(),std::ios::binary);
	if (!fout.is_open())
	{
