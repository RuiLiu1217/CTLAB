...
template<typename T>
std::vector<T> generateModiPhantom_template(
	...
	std::vector<T> phantom(totSize, 0);
	...
	CUDA_CHECK_RETURN(cudaMemcpy(&(phantom[0]), d_phantom, sizeof(T) * totSize, cudaMemcpyDeviceToHost));
	return phantom;
}


PhantomCreator::PhantomCreator() {

}

void PhantomCreator::create(int sizeX, int sizeY, int sizeZ) {
	data = generateModiPhantom_template<float>(sizeX, sizeY, sizeZ);
}

void  PhantomCreator::reset() {
	data.clear();
}

bool  PhantomCreator::save(std::string& fileName) {
	return writeData<float>(data, fileName);
}